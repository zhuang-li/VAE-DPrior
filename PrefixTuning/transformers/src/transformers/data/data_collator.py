...
@dataclass
class DataCollatorForWeightedLanguageModeling:
    """
    Data collator used for language modeling.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for masked language modeling
    ...
    def __call__(
        ...
        # print(examples[0])
        # print(len(examples))
        input_ids, labels, weights_lm, weights_discri = zip(*examples)
        # print(len(input_ids), len(labels), len(weights))

        batch = self._tensorize_batch(input_ids)
        labels = self._tensorize_batch(labels)
        # weights_lm = torch.tensor(weights_lm)
        weights_discri = torch.tensor(weights_discri)
        # print(weights_discri)
        best_lab = torch.max(weights_discri, dim=1)
        # print(best_lab)
        if self.tokenizer.pad_token_id is not None:
            labels[labels == self.tokenizer.pad_token_id] = -100
        # print(best_lab.indices)
        return {"input_ids": batch, "labels": labels, 'prompt_lab':best_lab.indices}
...
class DataCollatorForWeightedLanguageModeling_Old:
    ...
    def __call__(
        ...
        # print(examples[0])
        # print(len(examples))
        input_ids, labels, weights = zip(*examples)
        # print(len(input_ids), len(labels), len(weights))
        if self.mlm:
            inputs, labels = self.mask_tokens(batch)
            return {"input_ids": inputs, "labels": labels}
            ...
            if self.tokenizer.pad_token_id is not None:
                labels[labels == self.tokenizer.pad_token_id] = -100
            return {"input_ids": batch, "labels": labels, 'weights':weights}
...
class DataCollatorForEmbMatchLanguageModeling:
    ...
    def __call__(
        ...
        # print(examples[0])
        # print(len(examples))
        input_ids, labels, emb = zip(*examples)
        # print(len(input_ids), len(labels), len(weights))
        if self.mlm:
            inputs, labels = self.mask_tokens(batch)
            return {"input_ids": inputs, "labels": labels}
            ...
            if self.tokenizer.pad_token_id is not None:
                labels[labels == self.tokenizer.pad_token_id] = -100
            return {"input_ids": batch, "labels": labels, 'control_code':emb}
...
@dataclass
class DataCollatorForText2DataLanguageModeling:
    """
    Data collator used for language modeling.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for masked language modeling
...
@dataclass
class DataCollatorForTopicClusterLanguageModeling:
    """
    Data collator used for language modeling.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for masked language modeling
    ...
    def __call__(
        ...
        # print(examples[0])
        # print(len(examples))
        input_ids, labels, src, tgt, dis_labels, dis_src, dis_tgt = zip(*examples)
        ...
        if self.mlm:
            ...
            # print(self.format_mode, mode_input)
            first_src_length = src[0].size(0)
            if mode_input == 1:
                # input, batch
                batch = self._tensorize_batch(input_ids, self.tokenizer)
                labels = self._tensorize_batch(labels, self.tokenizer)
                #if first_src_length == 0:
                #    src = None
                #else:
                src = self._tensorize_batch(src, self.tokenizer)
                tgt = self._tensorize_batch(tgt, self.tokenizer)

                dis_src = self._tensorize_batch(dis_src, self.sentence_tokenizer)
                dis_tgt = self._tensorize_batch(dis_tgt, self.sentence_tokenizer)
                dis_labels = torch.LongTensor(dis_labels).squeeze()
                #dis_labels[dis_labels>=0] = 1
            ...
            tgt_attn = (tgt != self.tokenizer.pad_token_id) # tgt

            dis_tgt_attn = (dis_tgt != self.sentence_tokenizer.pad_token_id)
            dis_src_attn = (dis_src != self.sentence_tokenizer.pad_token_id)
            return {"input_ids": batch, "input_attn":input_attn, "labels": labels, 'src_attn': src_attn, 'tgt_attn':tgt_attn,
                    'src':src, 'tgt':tgt, 'dis_labels': dis_labels, 'dis_tgt': dis_tgt, 'dis_src': dis_src, 'dis_tgt_attn': dis_tgt_attn, 'dis_src_attn':dis_src_attn}
...
@dataclass
class DataCollatorForProtoClusterLanguageModeling:
    """
    Data collator used for language modeling.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for masked language modeling
    ...
    def __call__(
        ...
        input_ids, labels, src, tgt, dis_labels, dis_src, dis_tgt, dis_distributions = zip(*(flat_examples))
        
        #print (len(input_ids))
        #print(len(input_ids[0]))
        
        #input_ids = [item for sublist in nested_input_ids for item in sublist]
        #labels = [item for sublist in nested_labels for item in sublist]
        #src = [item for sublist in nested_src for item in sublist]
        #tgt = [item for sublist in nested_tgt for item in sublist]
        #dis_labels = [item for sublist in nested_dis_labels for item in sublist]
        #dis_src = [item for sublist in nested_dis_src for item in sublist]
        #dis_tgt = [item for sublist in nested_dis_tgt for item in sublist]

        #input_ids = [item for sublist in nested_dis_distributions for item in sublist]
        #print(labels[0].size())
        # print(len(input_ids), len(labels), len(weights))
        ...
        if mode_input == 1:
            ...
            dis_src = self._tensorize_batch(dis_src, self.sentence_tokenizer)
            dis_tgt = self._tensorize_batch(dis_tgt, self.sentence_tokenizer)
            dis_distributions = self._tensorize_batch(dis_distributions, self.sentence_tokenizer)
            dis_labels = torch.LongTensor(dis_labels).squeeze()
            #dis_labels[dis_labels>=0] = 1
        ...
        dis_tgt_attn = (dis_tgt != self.sentence_tokenizer.pad_token_id)
        dis_src_attn = (dis_src != self.sentence_tokenizer.pad_token_id)
        return {"input_ids": batch, "input_attn": input_attn,"tgt":tgt ,"labels": labels, 'src_attn': src_attn, 'tgt_attn':tgt_attn,
                'src':src, 'dis_labels': dis_labels, 'dis_tgt': dis_tgt, 'dis_src': dis_src, 'dis_distributions': dis_distributions, 'dis_tgt_attn': dis_tgt_attn, 'dis_src_attn': dis_src_attn}
...
@dataclass
class DataCollatorForCLModeling:
    """
    Data collator used for language modeling.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for masked language modeling
    ...
    def __call__(
        ...
        # print(examples[0])
        # print(len(examples))
        input_ids, labels, src, tgt, dis_labels, dis_tgt = zip(*examples)
        # print(len(input_ids), len(labels), len(weights))
        if self.mlm:
            inputs, labels = self.mask_tokens(batch)
            return {"input_ids": inputs, "labels": labels}
            ...
            #print(dis_tgt)
            return {"input_ids": batch, "labels": labels, 'src_attn': src_attn, 'tgt_attn':tgt_attn,
                    'src':src, 'dis_labels': dis_labels, 'dis_tgt': dis_tgt}
...
@dataclass
class DataCollatorForWritingPromptsLanguageModeling:
    """
    Data collator used for language modeling.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for masked language modeling
...
@dataclass
class DataCollatorForTopicLanguageModeling:
    """
    Data collator used for language modeling.
    - collates batches of tensors, honoring their tokenizer's pad_token
    - preprocesses batches for masked language modeling
    ...
    def __call__(
        ...
        # print(examples[0])
        # print(len(examples))
        input_ids, labels, control_code = zip(*examples)
        # print(len(input_ids), len(labels), len(weights))

        batch = self._tensorize_batch(input_ids)
        labels = self._tensorize_batch(labels)
        control_code = self._tensorize_batch(control_code)
        print(control_code)
        if self.tokenizer.pad_token_id is not None:
            labels[labels == self.tokenizer.pad_token_id] = -100
        return {"input_ids": batch, "labels": labels, "control_code":control_code}
...
class DataCollatorForClassificationSentimentLanguageModeling:
    ...
    def __call__(
        ...
        if self.tokenizer.pad_token_id is not None:
            labels[labels == self.tokenizer.pad_token_id] = -100
            # control_codes[control_codes==self.tokenizer.pad_token_id] = -100
        return {"input_ids": batch, "labels": labels}
...
class DataCollatorForKeywordLanguageModeling:
    ...
    def __call__(
        ...
        input_ids, labels, control_codes = zip(*examples)

        batch = self._tensorize_batch(input_ids)
        labels = self._tensorize_batch(labels)
        control_codes = self._tensorize_batch(control_codes)

        if self.tokenizer.pad_token_id is not None:
            labels[labels == self.tokenizer.pad_token_id] = -100
            # control_codes[control_codes==self.tokenizer.pad_token_id] = -100
        return {"input_ids": batch, "labels": labels, "control_code":control_codes}
