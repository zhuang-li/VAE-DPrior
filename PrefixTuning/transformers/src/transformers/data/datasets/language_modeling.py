...
class LineByLineSumTextDataset(Dataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, sentence_tokenizer: PreTrainedTokenizer, file_path: str, block_size: int, bos_tok:str, eos_tok:str,
        ...
        self.src_file = '{}/trn.source'.format(file_path)
        self.tgt_file = '{}/trn.target'.format(file_path)
    ...
    def __getitem__(self, i):
        ...
        #print(source_line)
        dis_label, src_line = source_line.split('\t')

        src = self.tokenizer(src_line, add_special_tokens=True, truncation=True, max_length=self.max_source_length,
                                 is_split_into_words=False)['input_ids']

        tgt = self.tokenizer(tgt_line, add_special_tokens=True, truncation=True, max_length=self.max_target_length,
                                 is_split_into_words=False)['input_ids']
        dis_tgt = self.sentence_tokenizer(tgt_line, add_special_tokens=True, truncation=True, max_length=self.max_target_length,
                                 is_split_into_words=False)['input_ids']
        # print(src, tgt)
        ...
        return (torch.tensor(sent, dtype=torch.long),
                ...
                int(dis_label),
                torch.tensor(dis_tgt, dtype=torch.long),
                )



class LineByLineUnderstandTextDataset(Dataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __getitem__(self, i):
        ...
        dis_tgt = self.sentence_tokenizer(tgt_line, add_special_tokens=True, truncation=True, max_length=self.max_target_length,
                                 is_split_into_words=False)['input_ids']
        # print(src, tgt)
        sent = tgt + [self.bos_idx] + src + [self.eos_idx]
        # print(sent)
...
class LineByLineCLDataset(Dataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """

    def __init__(self, tokenizer: PreTrainedTokenizer, sentence_tokenizer, label_id_List: List, label_list: List, sentence_list: List, block_size: int, bos_tok:str, eos_tok:str,
                 max_source_length:int, max_target_length:int, ):
        # assert os.path.isfile(file_path), f"Input file path {file_path} not found"
        # Here, we do not cache the features, operating under the assumption
        # that we will soon use fast multithreaded tokenizers from the
        ...
        for src_index, src_line in enumerate(label_list):
            # print(source_line)
            tgt_line = sentence_list[src_index]

            dis_label = label_id_List[src_index]
            ...
            tokens = self.sentence_tokenizer.tokenize(tgt_line)

            dis_tgt = self.sentence_tokenizer.convert_tokens_to_ids(tokens, unk_id=self.sentence_tokenizer.vocab['[UNK]'])
            #print(dis_tgt)
            self.dis_tgts.append(dis_tgt)
            # print(src, tgt)
            sent = src + [self.bos_idx] + tgt + [self.eos_idx]
            ...
            tgt_sent = sent[sep_idx - 1:]
            self.tgt_sents.append(tgt_sent)

        index_arr = np.arange(len(self.sents))
        np.random.shuffle(index_arr)
        index_arr_ids = list(index_arr)
        self.sents = [self.sents[i] for i in index_arr_ids]
        self.labels = [self.labels[i] for i in index_arr_ids]
        self.src_sents = [self.src_sents[i] for i in index_arr_ids]
        self.tgt_sents = [self.tgt_sents[i] for i in index_arr_ids]
        self.dis_labels = [self.dis_labels[i] for i in index_arr_ids]
        self.dis_tgts = [self.dis_tgts[i] for i in index_arr_ids]
    ...
    def __getitem__(self, i):
        ...
        return (torch.tensor(self.sents[i], dtype=torch.long),
                torch.tensor(self.labels[i], dtype=torch.long),
                torch.tensor(self.src_sents[i], dtype=torch.long),
                torch.tensor(self.tgt_sents[i], dtype=torch.long),
                int(self.dis_labels[i]),
                torch.tensor(self.dis_tgts[i], dtype=torch.long),
                )


class LineByLineTopicDataset(Dataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, sentence_tokenizer, label_id_List: List, label_list: List, sentence_list: List, block_size: int, bos_tok:str, eos_tok:str,
        ...
        for src_index, src_line in enumerate(label_list):
            ...
            dis_src = self.sentence_tokenizer(src_line, add_special_tokens=True, truncation=True,
                                              max_length=self.max_target_length,
                                              is_split_into_words=False)['input_ids']
            #print(dis_tgt)
            self.dis_srcs.append(dis_src)

            dis_tgt = self.sentence_tokenizer(tgt_line, add_special_tokens=True, truncation=True,
                                              max_length=self.max_target_length,
                                              is_split_into_words=False)['input_ids']
            #print(dis_tgt)
            self.dis_tgts.append(dis_tgt)
        ...
        self.tgt_sents = [self.tgt_sents[i] for i in index_arr_ids]
        self.dis_labels = [self.dis_labels[i] for i in index_arr_ids]
        self.dis_srcs = [self.dis_srcs[i] for i in index_arr_ids]
        self.dis_tgts = [self.dis_tgts[i] for i in index_arr_ids]
    ...
    def __getitem__(self, i):
        ...
        return (torch.tensor(self.sents[i], dtype=torch.long),
                ...
                torch.tensor(self.tgt_sents[i], dtype=torch.long),
                int(self.dis_labels[i]),
                torch.tensor(self.dis_srcs[i], dtype=torch.long),
                torch.tensor(self.dis_tgts[i], dtype=torch.long),
                )



class LineByLineProtoDataset(Dataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, sentence_tokenizer, label_id_List: List, label_list: List, sentence_list: List, block_size: int, bos_tok:str, eos_tok:str,
                 max_source_length:int, max_target_length:int, iterations: int, classes_per_it:int, sample_per_class:int):
        # assert os.path.isfile(file_path), f"Input file path {file_path} not found"
        # Here, we do not cache the features, operating under the assumption
        # that we will soon use fast multithreaded tokenizers from the
        # `tokenizers` repo everywhere =)
        ...
        for src_index, src_line in enumerate(label_list):
            ...
            #print(src_index)
            assert isinstance(tgt_line, str) , "{0} is not a string".format(src_index)
            tgt = self.tokenizer(tgt_line, add_special_tokens=True, truncation=True, max_length=self.max_target_length,
                                 is_split_into_words=False)['input_ids']
            ...
            label_distribution = torch.zeros(self.sentence_tokenizer.vocab_size)

            unique_label_ids, label_count = torch.unique(torch.tensor(dis_tgt, dtype=torch.long), return_counts=True)
            label_distribution[unique_label_ids] = label_count.float()

            label_distribution = label_distribution / torch.sum(label_distribution)
            self.dis_distributions.append(label_distribution)
            #print(dis_tgt)
            self.dis_tgts.append(dis_tgt)
            # print(src, tgt)
            sent = src + [self.bos_idx] + tgt + [self.eos_idx]
        ...
        #label_dict = {}
        #for indx, label in enumerate(self.dis_labels):
        ...
        self.dis_distributions = [self.dis_distributions[i] for i in index_arr_ids]

        self.dis_labels = self.dis_labels
        self.classes_per_it = classes_per_it
        self.sample_per_class = sample_per_class
        self.iterations = iterations

        self.classes, self.counts = np.unique(self.dis_labels, return_counts=True)
        ...
        self.idxs = range(len(self.dis_labels))
        self.indexes = np.empty((len(self.classes), max(self.counts)), dtype=int) * np.nan
        self.indexes = torch.Tensor(self.indexes)
        self.numel_per_class = torch.zeros_like(self.classes)
        for idx, label in enumerate(self.dis_labels):
            label_idx = np.argwhere(self.classes == label).item()
            self.indexes[label_idx, np.where(np.isnan(self.indexes[label_idx]))[0][0]] = idx
            self.numel_per_class[label_idx] += 1
        #print(self.numel_per_class)
        #print(self.indexes.shape)
        self.np_prob = self.counts / self.counts.sum()
        #print(self.counts)
    ...
    def __getitem__(self, it):
        ...
        #np_counts = self.counts.numpy()

        #np_prob = np_counts / np_counts.sum()

        np_indx = np.random.choice(len(self.classes), size=cpi, replace=False, p=self.np_prob)
        #print(c_idxs)
        for i, c in enumerate(self.classes[torch.from_numpy(np_indx)]):
        ...
        batch_list = batch[torch.randperm(len(batch))].tolist()
        return [(torch.tensor(self.sents[batch_indx], dtype=torch.long),
                torch.tensor(self.labels[batch_indx], dtype=torch.long),
                torch.tensor(self.src_sents[batch_indx], dtype=torch.long),
                torch.tensor(self.tgt_sents[batch_indx], dtype=torch.long),
                int(self.dis_labels[batch_indx]),
                torch.tensor(self.dis_srcs[batch_indx], dtype=torch.long),
                torch.tensor(self.dis_tgts[batch_indx], dtype=torch.long),
                self.dis_distributions[batch_indx]) for batch_indx in batch_list]



class LineByLinePrivacyDataset(Dataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, sentence_tokenizer, label_id_List: List, label_list: List, sentence_list: List, block_size: int, bos_tok:str, eos_tok:str,
        ...
        for src_index, src_line in enumerate(label_list):
            # print(source_line)
            input_tgt_line, output_tgt_line = sentence_list[src_index]

            #print(input_tgt_line)
            #print(output_tgt_line)
            ...
            tgt = self.tokenizer(output_tgt_line, add_special_tokens=True, truncation=True, max_length=self.max_target_length,
                                 is_split_into_words=False)['input_ids']
            ...
            dis_tgt = self.sentence_tokenizer(input_tgt_line, add_special_tokens=True, truncation=True,
                                              max_length=self.max_target_length,
                                              is_split_into_words=False)['input_ids']
            #print(dis_tgt)
            self.dis_tgts.append(dis_tgt)
...
class LineByLineNoRelDataset(Dataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, sentence_tokenizer, label_id_List: List, label_list: List, sentence_list: List, block_size: int, bos_tok:str, eos_tok:str,
        ...
        for src_index, src_line in enumerate(label_list):
            ...
            self.dis_tgts.append(dis_tgt)
            # print(src, tgt)
            sent = [self.bos_idx] + tgt + [self.eos_idx]
            self.sents.append(sent)
            # print(sent)
...
class LineByLineUnderstandFrontTextDataset(Dataset):
    """
    This will be superseded by a framework-agnostic approach
    soon.
    """
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, sentence_tokenizer: PreTrainedTokenizer, file_path: str, block_size: int, bos_tok:str, eos_tok:str,
                 max_source_length:int, max_target_length:int, noise:float):
        # assert os.path.isfile(file_path), f"Input file path {file_path} not found"
        # Here, we do not cache the features, operating under the assumption
        # that we will soon use fast multithreaded tokenizers from the
        # `tokenizers` repo everywhere =)
        ...
        assert len(tgt_lines) == len(src_lines)
        dis_label_dict = {}
        for src_index, source_line in enumerate(src_lines):
            # print(source_line)
            tgt_line = tgt_lines[src_index]
            dis_label, src_line = source_line.split('\t')
            ...
            label = copy.deepcopy(sent)
            self.labels.append(label)
            # label[sep_idx:] = [-100] * (len(label) - sep_idx)
            src_sent = sent[:sep_idx - 1]
            self.src_sents.append(src_sent)
            tgt_sent = sent[sep_idx - 1:]
            self.tgt_sents.append(tgt_sent)

        #dis_label_list = list(dis_label_set)
        if self.noise > 0.0:
            noise_sents = []
            noise_labels = []
            noise_src_sents = []
            ...
            label_length = len(dis_label_dict.keys()) - 1
            for src_index, sent in enumerate(self.sents):
                rand_num = random.uniform(0, 1)
                if rand_num < self.noise:

                    noise_dis_label = randint(0, label_length)
                    noise_dis_label = str(noise_dis_label)

                    dis_label = self.dis_labels[src_index]

                    while noise_dis_label == dis_label:
                        noise_dis_label = randint(0, label_length)
                        noise_dis_label = str(noise_dis_label)


                    dis_tgt = self.dis_tgts[src_index]

                    sent = self.sents[src_index]

                    sep_idx = sent.index(self.bos_idx) + 1

                    noise_src = dis_label_dict[noise_dis_label]

                    tgt = sent[sep_idx:]

                    noise_sent = noise_src + [self.bos_idx] + tgt
                    noise_sents.append(noise_sent)
                    #label = self.labels[src_index]

                    noise_label = copy.deepcopy(noise_sent)
                    noise_sep_idx = noise_sent.index(self.bos_idx) + 1
                    noise_label[noise_sep_idx:] = [-100] * (len(noise_label) - noise_sep_idx)
                    noise_labels.append(noise_label)

                    noise_src_sent = noise_sent[:noise_sep_idx - 1]
                    noise_src_sents.append(noise_src_sent)
                    noise_tgt_sent = noise_sent[noise_sep_idx - 1:]
                    noise_tgt_sents.append(noise_tgt_sent)
                    noise_dis_labels.append(noise_dis_label)
                    noise_dis_tgts.append(dis_tgt)
                    #print(label_length)
                    #print(sent)
                    #print(noise_sent)

            self.sents.extend(noise_sents)
            self.labels.extend(noise_labels)
            self.src_sents.extend(noise_src_sents)
            self.tgt_sents.extend(noise_tgt_sents)
            self.dis_labels.extend(noise_dis_labels)
            self.dis_tgts.extend(noise_dis_tgts)
