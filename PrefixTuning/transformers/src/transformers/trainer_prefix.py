...
class Trainer_Prefix:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.gpt2 = model.gpt2_model
        if self.model.encoder_config['disentangle_loss'] == 4:
            #print ("hidden size")
            #print (self.model.encoder_config['hidden_size'])
            self.adv_label_classifier = nn.Linear(512, self.model.class_num)
            self.adv_label_classifier = self.adv_label_classifier.to(args.device)
            self.adv_optimizer = torch.optim.Adam(self.adv_label_classifier.parameters(),lr=self.args.learning_rate)
    ...
    def compute_loss(self, model, inputs, gpt2_model=None):
        ...
        if hasattr(model, "module"):
            config_model = model.module
        else:
            config_model = model
        
        aug_iter = int(config_model.topic_encoder.config['aug_iter']/torch.cuda.device_count())
    #config['aug_iter'] = int(config['aug_iter']/config['n_gpu'])
        #print (self.global_step)
        #print(type(config_model))
        if config_model.sentence_encoder is not None and config_model.sentence_encoder.vae in ['ivae','ivae_nocond','ivae_nocond_fix']:
            if config_model.topic_encoder.config['pretrain_coeff'] * aug_iter == self.global_step:
                if config_model.sentence_encoder.vae == 'ivae':
                    if len(config_model.topic_encoder.train_questions) > config_model.topic_encoder.config['topic_num']:
                        print("initilize the topic embedding")
                        config_model.topic_encoder.initilize_topic_embedding(
                            config_model.topic_encoder.sentence_tokenizer, config_model.topic_encoder.train_questions,
                            config_model.topic_encoder.config['topic_num'],
                            config_model.topic_encoder.topic_embedding.weight.device)
                    #else:
                    #    config_model.topic_encoder.topic_embedding.weight.requires_grad = False
                elif config_model.sentence_encoder.vae == 'ivae_nocond_fix':
                    config_model.topic_encoder.topic_embedding.weight.requires_grad = False
                config_model.topic_encoder.starting_flag = True
                config_model.sentence_encoder.starting_flag = True
                #config_model.topic_encoder.get_topic_embedding()

        if self.task_mode == 'casual_lens':
            if config_model.topic_encoder.config['pretrain_coeff'] * aug_iter == self.global_step:
                config_model.topic_encoder.starting_flag = True
        
        if self.task_mode in ['topic', 'no_rel', 'proto', 'proto_reg', 'no_hidden', 'privacy','casual_lens']:
            
            if config_model.encoder_config['disentangle_loss'] == 4:
                self.adv_optimizer.zero_grad()
                outputs, topic_loss, topic_embed, dis_labels = model(**inputs)
                #print (dis_labels)
                gen_logits = self.adv_label_classifier(topic_embed.squeeze())
                adv_logits = self.adv_label_classifier(topic_embed.squeeze().detach())
                adv_style_loss = F.cross_entropy(adv_logits, dis_labels)
                disentangle_loss = -F.cross_entropy(gen_logits, dis_labels)
                topic_loss = topic_loss + disentangle_loss
                adv_style_loss.backward()
                self.adv_optimizer.step()
            else:
                outputs, topic_loss = model(**inputs, alpha = self.alpha)
        else:
        ...
        return outputs.loss + topic_loss
    ...
    def _save(self, output_dir: Optional[str] = None):
        ...
        if self.tokenizer is not None:
            self.tokenizer.save_pretrained(output_dir)
        #torch.save(self.topic_encoder, os.path.join(output_dir, "topic_model.pt"))
        #torch.save(self.sentence_encoder, os.path.join(output_dir, "label_model.pt"))
